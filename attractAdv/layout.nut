...
class UserConfig </ help="Navigation controls: Up/Down (to move up and down) and Page Up/Page Down (to move left and right)" />{
	...
	</ label="해상도 Width", help="가로 해상도", option="1024", order=15 />  user_width="320";
	</ label="해상도 Height", help="세로 해상도" option="768", order=16 />  user_height="240";
...
//fe.load_module("objects/scrollingtext");		// 미사용 텍스트 스크롤링 모듈
...
{	
	...
	{		
		...
		{	
			...
			sel_snap.y = fe.layout.height / 30 + height * sel_y + PAD;	// 선택 GRID snap 위치 y
		...
		frame.y = fe.layout.height / 30 + height * sel_y;			// GRID 선택 Frame 위치 y
		frame.zorder=100;						// GRID 선택 zorder

		overlay.x = width * sel_x;					// GRID 선택 Frame 위치 x
		overlay.y = fe.layout.height / 35 + height * sel_y + height - height/8;			// GRID 선택 Frame 위치 y
...
class MySlot extends ConveyorSlot
{
	...
	constructor( num )
	{
		...
		if( my_config["ListEntryYn"] == "Y")
			m_ttt = fe.add_text( " [ListEntry]. [!title_format]", 0, 0, width - 2*PAD, height/7);
		else
			m_ttt = fe.add_text( " [!title_format]", 0, 0, width - 2*PAD, height/7);

		m_ttt.charsize = width/15;		
	...
	function on_progress( progress, var )
	{
		...
		else
		{
			...
			if ( abs( var ) < cols )
			{
				...
				m_art.y = fe.layout.height / 30 + r * height + PAD;
			}
			else
			{
				local prog = ::gridc.transition_progress;
		...
		m_ttt.x = m_art.x;
		m_ttt.y = m_art.y+height-(height/7);
...
local move_frame = {
    when =Transition.StartLayout ,property = "alpha", start = 100, end = 255, time = 200, tween = Tween.Linear,  pulse = true
 } 
...
gridc.num_t = fe.add_text( "[ListEntry] / [ListSize]",	fe.layout.width/2, 0, fe.layout.width/2, fe.layout.height / 30 );
gridc.num_t.align= Align.MiddleRight;
gridc.num_t.charsize = 10;
